...
print "TODO: Kommentar am Zeilenende bei export, versandkosten nicht mit --cache funktionierend, " \
      "Funktion um Mails zu schicken und VK AUftrag abzuschließen, " \
      "VK Auftrag vorher noch auf Entwurf lassen damit einfacher löschbar, exportNativeBasket nutzen"

AUTOIMPORT_CATEGORY_ID = categoryIdFromName('automagisch importiert')
...
if sys.argv[1] == "export":
    ...
    for currentShop in totalBasket.shops():
        order_data = getDefault('purchase.order',
                                ["origin", "message_follower_ids", "order_line", "company_id", "currency_id",
                                 "date_order", "location_id", "message_ids", "invoiced", "dest_address_id",
                                 "fiscal_position", "amount_untaxed", "partner_id", "journal_id", "amount_tax", "state",
                                 "pricelist_id", "warehouse_id", "payment_term_id", "partner_ref", "date_approve",
                                 "amount_total", "name", "notes", "invoice_method", "shipped", "validator",
        ...
        def addPartToPurchaseOrder(prod_id, count, price, purchaseOrderData):
            ...
            order_line_data = oerp.execute(
                ...
                False, prod_id, count, False, purchaseOrderData["partner_id"][0], purchaseOrderData["date_order"],
                purchaseOrderData["fiscal_position"], False, False, False)
        ...
        addPartToPurchaseOrder(SHIPPINGCOST_PRODUCT_ID, pricefetcher.shopByName(currentShop).shipping, 1.00,
                               order_data)  # HARDCODED
